    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            HStack(spacing: 12) {
                // Eraser Tool
                Button(action: { 
                    state.eraserToolActive.toggle()
                    if state.eraserToolActive {
                        state.brushToolActive = false
                ...
                // Brush Tool
                Button(action: { 
                    state.brushToolActive.toggle()
                    if state.brushToolActive {
                        state.eraserToolActive = false
                        DispatchQueue.main.async {
                            state.createCircularCursor(size: state.brushSize).set()
                        }
                    } else {
                    ...
                    HStack(spacing: 6) {
                        Image(systemName: "paintbrush.fill")
                            .foregroundStyle(state.brushToolActive ? .blue : .primary)
                        Text("Brush")
                    }
                }
                .buttonStyle(.bordered)
                .background(state.brushToolActive ? .blue.opacity(0.2) : .clear, in: RoundedRectangle(cornerRadius: 6))
                .disabled(state.dustMask == nil)
                .help("Click and add dust to mask with circular brush")

                // Brush Size Slider (visible when eraser or brush tool is active)
